...
public record HappinessRoutinesGetServiceResponse(
    @NonNull List<HappinessRoutineResponse> routines
) {

    public static HappinessRoutinesGetServiceResponse of(List<HappinessRoutine> routines) {
        return new HappinessRoutinesGetServiceResponse(routines.stream().map(HappinessRoutineResponse::of).toList());
    }
