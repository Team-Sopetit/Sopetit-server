...
public record HappinessRoutinesServiceResponse(
    @NonNull List<HappinessRoutineResponse> routines
) {

    public static HappinessRoutinesServiceResponse of(List<HappinessRoutine> routines) {
        return new HappinessRoutinesServiceResponse(routines.stream().map(HappinessRoutineResponse::of).toList());
    }
